    ...
    <div class="page-header d-print-none mb-4">
        <div class="row align-items-center">
            <div class="col">
                ...
                <h2 class="page-title">
                    {{ Str::ucfirst(Str::lower($post['topicType'] ?? 'Post')) }}
                </h2>
            </div>
            <div class="col-auto">
                @switch($post['state'] ?? '---')
                    @case('PROCESSING')
                        <span class="badge bg-yellow text-yellow-fg badge-lg">{{ Str::ucfirst(Str::lower($post['state'] ?? '---')) }}</span>
                        @break
                    @case('LIVE')
                        <span class="badge bg-green text-green-fg badge-lg">{{ Str::ucfirst(Str::lower($post['state'] ?? '---')) }}</span>
                        @break
                    @default
                        <span class="badge bg-red text-red-fg badge-lg">{{ Str::ucfirst(Str::lower($post['state'] ?? '---')) }}</span>
                @endswitch
            </div>
        </div>
    </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        <span class="avatar avatar-sm me-3 rounded">
                        ...
                        <h3 class="card-title mb-0">{{ Str::ucfirst(Str::lower($post['topicType'])) }}</h3>
                ...
                <div class="card-footer">
                    <div class="btn-list">
                        <a href="{{ $post['callToAction']['url'] ?? '#' }}" title="{{ Str::ucfirst(Str::lower(Str::replace('_', ' ', $post['callToAction']['actionType'] ?? 'LEARN_MORE'))) }}" target="_blank" class="btn btn-clnkgo">
                        ...
                        <form action="{{ route('dashboard.posts.delete', ['id' => last(explode('/', $post['name'] ?? ''))]) }}" method="POST" style="display: inline-block;">
                            @method('DELETE')
                            @csrf
                            <button type="submit" class="btn btn-outline-danger" title="Delete" onclick="return confirm('Voulez-vous vraiment supprimer cet article ?')">
                            ...
                            <ul class="splide__list">
                                @foreach(($post['media'] ?? []) as $index => $media)
                                    <li class="splide__slide">
                                        <img src="{{ $media['googleUrl'] ?? '#' }}" alt="" style="border-radius: 4px;"/>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
