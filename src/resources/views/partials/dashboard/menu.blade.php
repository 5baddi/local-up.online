                ...
                @if(request()->routeIs(['dashboard', 'dashboard.*']))
                    ...
                    <li class="nav-item {{ request()->routeIs(['dashboard.reviews', 'dashboard.reviews.*']) ? 'active' : '' }}">
                        <a class="nav-link" href="{{ route('dashboard.reviews') }}">
                            <span class="nav-link-icon d-md-none d-lg-inline-block">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-map-star"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M9.718 17.359l-.718 -.359l-6 3v-13l6 -3l6 3l6 -3v7.5"/><path d="M9 4v13"/><path d="M15 7v4"/><path d="M17.8 20.817l-2.172 1.138a.392 .392 0 0 1 -.568 -.41l.415 -2.411l-1.757 -1.707a.389 .389 0 0 1 .217 -.665l2.428 -.352l1.086 -2.193a.392 .392 0 0 1 .702 0l1.086 2.193l2.428 .352a.39 .39 0 0 1 .217 .665l-1.757 1.707l.414 2.41a.39 .39 0 0 1 -.567 .411l-2.172 -1.138z"/></svg>
                            </span>
                            <span class="nav-link-title">{{ trans('dashboard.reviews') }}</span>
                        </a>
                    </li>

                    <li class="nav-item nav-item-divider {{ request()->routeIs('dashboard.account') ? 'active' : '' }}">
                        <a class="nav-link" href="{{ route('dashboard.account') }}">
                            <span class="nav-link-icon d-md-none d-lg-inline-block">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-settings"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10.325 4.317c.426 -1.756 2.924 -1.756 3.35 0a1.724 1.724 0 0 0 2.573 1.066c1.543 -.94 3.31 .826 2.37 2.37a1.724 1.724 0 0 0 1.066 2.573c1.756 .426 1.756 2.924 0 3.35a1.724 1.724 0 0 0 -1.066 2.573c.94 1.543 -.826 3.31 -2.37 2.37a1.724 1.724 0 0 0 -2.573 1.066c-.426 1.756 -2.924 1.756 -3.35 0a1.724 1.724 0 0 0 -2.573 -1.066c-1.543 .94 -3.31 -.826 -2.37 -2.37a1.724 1.724 0 0 0 -1.066 -2.573c-1.756 -.426 -1.756 -2.924 0 -3.35a1.724 1.724 0 0 0 1.066 -2.573c-.94 -1.543 .826 -3.31 2.37 -2.37a1.724 1.724 0 0 0 2.573 -1.066z"/><path d="M9 12a3 3 0 1 0 6 0a3 3 0 0 0 -6 0"/></svg>
                            </span>
                            <span class="nav-link-title">{{ trans('global.account') }}</span>
                        </a>
                    </li>
                @endif
            </ul>
            <div class="sidebar-footer mt-auto">
                <div class="sidebar-user-info">
                    <div class="d-flex align-items-center px-3 py-2">
                        <span class="avatar avatar-sm avatar-rounded" style="background-image: url({{ $avatar }})"></span>
                        <div class="ps-2 text-truncate">
                            <div class="sidebar-user-name">{{ $user->getFullName() }}</div>
                            <div class="sidebar-user-email text-muted small">{{ $user->email }}</div>
                        </div>
                    </div>
                </div>
                <div class="px-3 pb-3">
                    <a href="{{ route('signout') }}" class="btn btn-clnkgo w-100">
